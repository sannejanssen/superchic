	...
	Sliderman.slider = function(parameters){
		...
		for(var i = 0; i < mainCont.childNodes.length; i++) if(mainCont.childNodes[i].nodeType == 1){
			if(contentmode) images.push(mainCont.childNodes[i].cloneNode(true));
				...
				switch(mainCont.childNodes[i].tagName)
				{
					...
					break;
					case 'IMG':
						// haal source uit de images in de html
						total_images++;
		...
		if(navigation)
		{
			...
			{	
				...
				// alert( descriptions[i].innerHTML );
				// alert( a.innerHTML );
				// a.innerHTML = "<span>test</span>";
				if(navigation.className) a.className = navigation.className;
				a.className = 'helvetica ';
				a.href = 'javascript:void(0);';
				...
				if( descriptions[i] )
				{
					...
					if( cur_cat == '')
					{
						cur_cat = descriptions[i].innerHTML;
						// categories[i] = descriptions[i].innerHTML;
						a.innerHTML = descriptions[i].innerHTML;
						navigationCont.appendChild(a);
					}
					if( cur_cat != '')
					{
						if( descriptions[i].innerHTML != cur_cat )
						{
							cur_cat = descriptions[i].innerHTML;
							// categories[i] = descriptions[i].innerHTML;
							a.innerHTML = descriptions[i].innerHTML;
			...
			*/
			...
			function navigationUpd()
			{
				...
				else
				{
					if(current)
					{
						if( current >= 1 && current <= total_images)
						{
							for(var i = current; i >=0; i-- )
							{
								if( descriptions[i] )
								{
									...
									{
										navigationLinks[j].className = navigationLinks[j].className.replace(/\bactive\b/g, '');
									}
									navigationLinks[i].className += ' active';
									break;
								}
							}
							// alert(current + " - " + total_images);
						}
						else
						{
